package us12;

public class AdministratorNotManageTests {
//    @Test
//    public void notManageEventsAdmin() {
//        checkPasswordAdmin();
//        driver.findElement(preLocation.getChooseLocationButton()).click();
//        driver.findElement(locationPage.getChooseLocationUkraine()).click();
//        driver.findElement(chooseCityUkraine.getButtonKiev()).click();
//        driver.findElement(preLocation.getNextButton()).click();
//        driver.findElement(mySpacePage.getRoomsImage()).click();
//        driver.findElement(mySpacePage.getChooseDataButton()).click();
//        driver.findElement(mySpacePage.getChooseEventButton()).click();

}
