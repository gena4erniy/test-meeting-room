...
public class DisplayingAReminderIconTest extends BaseTest {
    ...
    @Owner("Vaskovich K.")
    @TmsLink("5632888")
    @Test(description = "Verification that the reminder has a set time.")
    public void DisplayingAReminderIconTest() {
        Assert.assertTrue(driver.findElement(mySpacePage.getReminderBell()).isDisplayed() && driver.findElement(mySpacePage.getReminderTime()).isDisplayed(), "The reminder has a set time.");
    }
}
